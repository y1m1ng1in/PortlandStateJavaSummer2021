...
public class Project4 {
    ...
    public static final String SEARCH_AND_PRINT_BOTH_ENABLED = "When -search is enabled, -print must not be enabled, " +
            "since nothing can be added to appointment book";
    ...
    public static void main(String... args) {
        ...
        if (argumentParser.isEnabled("-search")) {
            ...
            try {
                System.out.println(client.getAppointmentsByOwnerWithBeginInterval(arguments[0], lowerbound,
                        upperbound));
            } catch (IOException e) {
        ...
        if (arguments.length == 1) {
            ...
            try {
                System.out.println(client.getAppointmentBookByOwner(arguments[0]));
            } catch (IOException e) {
                error("While connecting to host, " + e.getMessage());
            } catch (ParserException e) {
                error("Cannot parse content gets returned from server as an appointment book: " + e.getMessage());
